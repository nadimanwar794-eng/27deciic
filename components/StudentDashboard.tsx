  ...
  return (
    <div>
        ...
        <div className={`grid ${isGameEnabled ? 'grid-cols-7' : 'grid-cols-6'} gap-1 bg-white p-2 rounded-xl border border-slate-200 shadow-sm mb-8 sticky top-20 z-20 overflow-x-auto`}>
            ...
            {settings?.weeklyTests?.some(t => t.isActive && t.classLevel === user.classLevel) && <button onClick={() => onTabChange('WEEKLY_TEST')} className={`flex flex-col items-center justify-center p-2 rounded-lg transition-all min-w-[60px] ${activeTab === 'WEEKLY_TEST' ? 'bg-gradient-to-br from-red-400 to-pink-500 text-white shadow-md' : 'text-slate-400 hover:bg-slate-50'}`}><Calendar size={18} className="mb-1" /><span className="text-[9px] font-bold uppercase">Exclusive MCQ</span></button>}
